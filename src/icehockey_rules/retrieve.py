...
from icehockey_rules.rulebook import get_inmem_chunked_iihf_rulebook_index, __version__ as parsing_version, \
...
def chunk_matches_to_rules_df(matches: list[str, Any]) -> pd.DataFrame:
    ...
    df = pd.DataFrame(
        [
            (inmem_chunked_iihf_rulebook_index[match["id"]]["rule_number"], match["score"])
